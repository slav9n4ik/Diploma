...
import ru.diploma.nodes.TVDUndefinedFunctionRootNode;

import java.util.logging.Level;

/**
 * Represents a TVD function. On the Truffle level, a callable element is represented by a
 * {@link RootCallTarget call target}. This class encapsulates a call target, and adds version
 * <p>
 * The {@link #callTarget} can be {@code null}. To ensure that only one {@link TVDFunction} instance
 * per name exists, the {@link TVDFunctionRegistry} creates an instance also when performing name
 * lookup. A function that has been looked up, i.e., used, but not defined, has a call target that
 * encapsulates a {@link TVDUndefinedFunctionRootNode}.
 */
@ExportLibrary(InteropLibrary.class)
public class TVDFunction implements TruffleObject {
    public static final int INLINE_CACHE_SIZE = 2;

    private static final TruffleLogger LOG = TruffleLogger.getLogger(TVDLanguage.ID, TVDFunction.class);
    ...
    protected TVDFunction(TVDLanguage language, String name) {
        this.name = name;
        this.callTarget = Truffle.getRuntime().createCallTarget(new TVDUndefinedFunctionRootNode(language, name));
     ...
     * changing it has an effect on TVD programs.
     */
    @Override
    ...
    /**
     * {@link TVDFunction} instances are always visible as executable to other languages.
     */
    ...
    abstract static class Execute {
         ...
         *
         * <p>
         * Since TVD is a quite simple language, the benefit of the inline cache seems small: after
         ...
         * target is usually much more complicated than in TVD.
         * </p>
         *
         * <p>
         ...
         * function is redefined, the call target maintained by the TVDFunction object is changed. To
         * avoid a check for that, we use an Assumption that is invalidated by the TVDFunction when
         * the change is performed. Since checking an assumption is a no-op in compiled code, the
         * assumption check performed by the DTVD does not add any overhead during optimized
         * execution.
         * </p>
         *
         * @see Cached
        ...
        protected static Object doDirect(TVDFunction function, Object[] arguments,
        ...
        protected static Object doIndirect(TVDFunction function, Object[] arguments,
                                           @Cached IndirectCallNode callNode) {
            /*
             * TVD has a quite simple call lookup: just ask the function for the current call target,
             * and call it.
             */
